...
function setPreset(days) { document.getElementById('startDate').value=daysAgoKST(days-1); document.getElementById('endDate').value=todayKST(); applyFilter(); }
...
function mkChart(id,type,labels,datasets,opts={}) {
  ...
  CHARTS[id]=new Chart(ctx,{type,data:{labels,datasets},options:{
    ...
    scales:type==='doughnut'||type==='pie'?{}:{x:{grid:{display:false},ticks:{font:{size:11},maxRotation:45}},y:{beginAtZero:true,grid:{color:'#f0f0f0'}}},...opts
  }});
}
function paginator(cid,page,total,per,fn) {
  ...
  for(let p=Math.max(1,page-2);p<=Math.min(tp,page+2);p++) h+=`<button class="${p===page?'btn':'btn-ghost'}" onclick="${fn}(${p})">${p}</button>`;
...
function statusBadge(s) {
  ...
  const sc=s.trim();
  if(['DONE','결제 완료','결제완료','DONE 완료'].includes(sc)) return `<span class="badge badge-success">✅ 성공</span>`;
  if(sc.includes('CANCEL')||['참여취소','PARTIAL_CANCELED','제품회수'].includes(sc)) return `<span class="badge badge-cancel">❌ ${sc}</span>`;
  if(sc==='WAITING_FOR_DEPOSIT') return `<span class="badge badge-wait">⏳ 대기</span>`;
...
async function loadTrafficDetail() {
  ...
  const rows=d.data;
  if(!rows.length){document.getElementById('trafficTableWrap').innerHTML='<div class="no-data">기간 내 유입 데이터 없음</div>';return;}
  document.getElementById('trafficTableWrap').innerHTML=`
    ...
    <tbody>${rows.map(r=>`<tr>
      ...
      <td style="font-size:12px;word-break:break-all">${r.url?`<a href="${r.url}" target="_blank" style="color:#3182ce">${r.url}</a>`:'-'}</td>
    </tr>`).join('')}</tbody></table>`;
}
...
async function loadPayTable(page) {
  ...
  document.getElementById('payTableWrap').innerHTML=`
    ...
    <tbody>${d.data.map(r=>`<tr>
      ...
      <td>${r.refund===true?'<span class="badge badge-cancel">환불</span>':'<span style="color:#a0aec0;font-size:12px">-</span>'}</td>
    </tr>`).join('')}</tbody></table>`;
...
async function loadSimDetail() {
  ...
  jobEl.innerHTML=dist.job.slice(0,10).map(x=>{
    ...
    return `<div style="display:flex;align-items:center;margin-bottom:10px;">
      ...
      <div style="flex:1;background:#edf2f7;border-radius:4px;height:20px;overflow:hidden"><div style="width:${pct}%;height:100%;background:linear-gradient(90deg,#dd6b20,#f6ad55);display:flex;align-items:center;padding-left:6px"><span style="font-size:11px;color:white;font-weight:600">${pct}%</span></div></div>
...
async function loadSimTable(page) {
  ...
  let rows='';
  d.data.forEach((r,i)=>{
    rows+=`<tr><td style="white-space:nowrap">${r.created||'-'}</td><td>${r.name||'-'}</td><td style="font-size:12px;color:#718096">${r.email||'-'}</td><td>${r.job||'-'}</td><td>${r.company||'-'}</td>
      ...
      <td>${r.qa_count>0?`<button class="btn-sm" onclick="toggleQa('${r.session_id}',${i})">▼ Q&A</button>`:'<span style="color:#a0aec0;font-size:12px">-</span>'}</td></tr>
    <tr class="qa-expand-row" id="qr${i}"><td colspan="9"><div class="qa-inner" id="qi${i}"><div class="loading">불러오는 중...</div></div></td></tr>`;
  });
...
def api_summary():
    ...
    try:
        ...
        row = cur.fetchone(); pay_c, pay_a = row['c'], float(row['s'])
        days = (datetime.strptime(end,'%Y-%m-%d') - datetime.strptime(start,'%Y-%m-%d')).days + 1
        pe = (datetime.strptime(start,'%Y-%m-%d') - timedelta(days=1)).strftime('%Y-%m-%d')
        ps = (datetime.strptime(start,'%Y-%m-%d') - timedelta(days=days)).strftime('%Y-%m-%d')
        ...
        return jsonify({'login': login, 'prev_login': p_login,
                        'pay_count': pay_c, 'prev_pay_count': prow['c'],
                        'pay_amount': pay_a, 'prev_pay_amount': float(prow['s']),
                        'sim': sim, 'prev_sim': p_sim, 'qa': qa, 'prev_qa': p_qa})
    finally:
        conn.close()
...
def api_trend():
    ...
    try:
        ...
        def daily(table, dcol, extra=''):
            flt = date_filter(f'"{dcol}"', start, end)
            cur.execute(f'SELECT {ts_to_kst(f"{chr(34)}{dcol}{chr(34)}")}::date AS dt, COUNT(*) as c FROM "{SCHEMA}"."{table}" WHERE {flt} {extra} GROUP BY 1 ORDER BY 1')
        ...
        flt = date_filter('"Created Date"', start, end) + f' AND {PAY_STATUS_SUCCESS} AND {PAY_IM_FILTER}'
        cur.execute(f'SELECT {ts_to_kst(chr(34)+"Created Date"+chr(34))}::date AS dt, COALESCE(SUM(amount_number),0) as s FROM "{SCHEMA}"."custom$0__4" WHERE {flt} GROUP BY 1 ORDER BY 1')
        pay_a_d = {str(r['dt']): float(r['s']) for r in cur.fetchall()}
        result, d = [], datetime.strptime(start, '%Y-%m-%d')
...
def api_traffic_list():
    ...
    try:
        cur = conn.cursor()
        flt = ('WHERE ' + date_filter('"Created Date"', start, end)) if start and end else ''
        cur.execute(f'SELECT {ts_to_kst(chr(34)+"Created Date"+chr(34))} AS created, page_name_text AS page_name, url_text AS url FROM "{SCHEMA}"."custom$0_nlt_169_nlt_cust_traffic_info" {flt} ORDER BY "Created Date" DESC')
        rows = [{'created': str(r['created']) if r['created'] else None, 'page_name': r['page_name'], 'url': r['url']} for r in cur.fetchall()]
...
def api_login_list():
    ...
    q = request.args.get('q', ''); page = int(request.args.get('page', 1)); per = 50
    conn = get_conn()
    try:
        cur = conn.cursor()
        flt = date_filter('"Created Date"', start, end)
        if q: flt += f" AND email_text ILIKE '%{q}%'"
        cur.execute(f'SELECT COUNT(*) as c FROM "{SCHEMA}"."custom$0_nlt_136_changeup_login_out_info" WHERE {flt}')
        total = cur.fetchone()['c']
        cur.execute(f'SELECT email_text AS email, {ts_to_kst(chr(34)+"login_date_date"+chr(34))} AS login_dt, {ts_to_kst(chr(34)+"logout_date_date"+chr(34))} AS logout_dt, login_ip_text AS ip, unique_code_text AS code FROM "{SCHEMA}"."custom$0_nlt_136_changeup_login_out_info" WHERE {flt} ORDER BY "Created Date" DESC LIMIT {per} OFFSET {(page-1)*per}')
        rows = [{'email': r['email'], 'login_dt': str(r['login_dt']) if r['login_dt'] else None, 'logout_dt': str(r['logout_dt']) if r['logout_dt'] else None, 'ip': r['ip'], 'code': r['code']} for r in cur.fetchall()]
...
def api_payment_products():
    ...
    try:
        ...
        cur.execute(f'SELECT DISTINCT ____text AS p FROM "{SCHEMA}"."custom$0__4" WHERE {flt} AND ____text IS NOT NULL AND ____text != \'\' ORDER BY p')
...
def api_payment_list():
    start = request.args.get('start'); end = request.args.get('end')
    product = request.args.get('product', ''); status = request.args.get('status', '')
    refund = request.args.get('refund', ''); page = int(request.args.get('page', 1)); per = 50
    conn = get_conn()
    try:
        cur = conn.cursor()
        flt = date_filter('"Created Date"', start, end) + f' AND {PAY_IM_FILTER}'
        if product: flt += f" AND ____text = '{product.replace(chr(39), chr(39)*2)}'"
        if status == 'success': flt += f' AND {PAY_STATUS_SUCCESS}'
        elif status == 'cancel': flt += f' AND {PAY_STATUS_CANCEL}'
        elif status == 'wait': flt += f' AND {PAY_STATUS_WAIT}'
        if refund == 'true': flt += ' AND ______1_boolean = true'
        elif refund == 'false': flt += ' AND (______1_boolean = false OR ______1_boolean IS NULL)'
        ...
        agg = cur.fetchone(); total, total_amount = agg['c'], float(agg['s'])
        cur.execute(f'SELECT orderid_text AS order_id, TO_CHAR({ts_to_kst(chr(34)+"Created Date"+chr(34))}, \'YYYY-MM-DD HH24:MI:SS\') AS created, ____text AS product, __1_text AS buyer_name, ______text AS phone, ____1_text AS method, _____text AS status, ____3_text AS amount, code_string_text AS coupon, ______1_boolean AS refund FROM "{SCHEMA}"."custom$0__4" WHERE {flt} ORDER BY "Created Date" DESC LIMIT {per} OFFSET {(page-1)*per}')
        rows = []
        for r in cur.fetchall():
            try: amt_val = float(r['amount']) if r['amount'] else None
            except: amt_val = None
            rows.append({'order_id': r['order_id'], 'created': r['created'], 'product': r['product'],
                         'buyer_name': r['buyer_name'], 'phone': r['phone'], 'method': r['method'],
                         'status': r['status'], 'amount': amt_val, 'coupon': r['coupon'], 'refund': r['refund']})
...
def api_sim_list():
    start = request.args.get('start'); end = request.args.get('end')
    q = request.args.get('q', ''); product = request.args.get('product', '')
    page = int(request.args.get('page', 1)); per = 50
    conn = get_conn()
    try:
        cur = conn.cursor()
        flt = date_filter('s."Created Date"', start, end)
        if q: flt += f" AND (s.email_text ILIKE '%{q}%' OR s.name_text ILIKE '%{q}%')"
        if product: flt += f" AND s.product_text = '{product.replace(chr(39), chr(39)*2)}'"
        cur.execute(f'SELECT COUNT(*) as c FROM "{SCHEMA}"."custom$0_nlt_159_interview_simulation_data" s WHERE {flt}')
        total = cur.fetchone()['c']
        cur.execute(f'SELECT s._id AS session_id, TO_CHAR({ts_to_kst("s."+chr(34)+"Created Date"+chr(34))}, \'YYYY-MM-DD HH24:MI:SS\') AS created, s.name_text AS name, s.email_text AS email, s.job_text AS job, s.company_text AS company, s.product_text AS product, s.interview_mate_yn_boolean AS interview_mate, COUNT(q._id) AS qa_count FROM "{SCHEMA}"."custom$0_nlt_159_interview_simulation_data" s LEFT JOIN "{SCHEMA}"."custom$0_nlt_162_chatgpt_raw" q ON s._id = q.unique_code_text WHERE {flt} GROUP BY s._id, s."Created Date", s.name_text, s.email_text, s.job_text, s.company_text, s.product_text, s.interview_mate_yn_boolean ORDER BY s."Created Date" DESC LIMIT {per} OFFSET {(page-1)*per}')
        rows = [{'session_id': r['session_id'], 'created': r['created'], 'name': r['name'],
                 'email': r['email'], 'job': r['job'], 'company': r['company'],
                 'product': r['product'], 'interview_mate': r['interview_mate'], 'qa_count': r['qa_count']} for r in cur.fetchall()]
        return jsonify({'total': total, 'data': rows})
    finally:
        conn.close()
...
def api_session_qa():
    ...
    try:
        cur = conn.cursor()
        cur.execute(f'SELECT no__number AS no, question_category_text AS category, question_text AS question, answer_text AS answer, feedback_text AS feedback FROM "{SCHEMA}"."custom$0_nlt_162_chatgpt_raw" WHERE unique_code_text = %s ORDER BY no__number ASC NULLS LAST, "Created Date" ASC', (session_id,))
        rows = [{'no': int(r['no']) if r['no'] else None, 'category': r['category'],
                 'question': r['question'], 'answer': r['answer'], 'feedback': r['feedback']} for r in cur.fetchall()]
        return jsonify(rows)
    finally:
        conn.close()
